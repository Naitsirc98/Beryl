...
public class Tree {
    ...
    private static StaticMeshView getTrunkMeshView() {
        ...
        return new StaticMeshView(treeModel.mesh(name), PhongMaterial.getFactory().getMaterial(name, material -> {
            ...
            material.setColorMap(g.newTexture2D(getTexturePath("Bark_Color.png"), PixelFormat.RGBA).setQuality(Texture.Quality.MEDIUM));
        }));
    }
    ...
    private static StaticMeshView getCapMeshView() {
        ...
        return new StaticMeshView(treeModel.mesh(name), PhongMaterial.getFactory().getMaterial(name, material -> {
            GraphicsFactory g = GraphicsFactory.get();
            material.setColorMap(g.newTexture2D(getTexturePath("Cap_Color.png"), PixelFormat.RGBA).setQuality(Texture.Quality.LOW));
        }));
    }
    ...
    private static StaticMeshView getLeafMeshView() {
        ...
        return new StaticMeshView(treeModel.mesh(name), PhongMaterial.getFactory().getMaterial(name, material -> {
            GraphicsFactory g = GraphicsFactory.get();
            material.setColorMap(g.newTexture2D(getTexturePath("Leaf_Color.png"), PixelFormat.RGBA).setQuality(Texture.Quality.HIGH));
